...
@extends('vendor.log-viewer.bootstrap-3.master_log')
@section('title')
Quản lý sự cố hệ thống
@endsection
@section('url-home')
{{ route('admin.category.index')}}
@endsection
@section('breadcrumb')
    Chi tiết sự cố ngày [{{ $log->date }}]
@endsection
@section('content')
                                ...
                                <span class="input-group-btn">
                                    @unless (is_null($query))
                                        <a href="{{ route('log-viewer::logs.show', [$log->date]) }}" class="btn btn-outline-secondary">
                                            (@lang(':count results', ['count' => $entries->count()])) <i class="lni lni-close"></i>
                                        </a>
                                    @endunless
